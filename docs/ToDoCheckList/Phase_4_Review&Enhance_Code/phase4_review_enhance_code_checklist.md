...
| Silver transformation | Injectable job reads chunks, validates/quarantines, stages, globally deduplicates, validates, and publishes a versioned candidate | ✅ Correctness fixed (2026-09-09): staged content is rewritten with the deduplicated frame before publish. ✅ 6.5.7-6.5.10 (2026-09-09): optional `dedup_service`/`sql_validator`/`staging_reader` injection lets SQL (`PostgresSilverDedupService`, `PostgresSilverStagingValidator`) own dedup and validation on the staged Postgres table directly; wired behind `Settings.silver_sql_dedup_enabled` (default `False`); **proven against a real live-PostgreSQL staging table** (`tests/test_postgres_silver_sql_dedup_validator_integration.py`, 6 tests) — an earlier assumption in this doc that "no live PostgreSQL" was available was checked and found wrong. The integration test caught and fixed a real bug: `count(*) - count(DISTINCT pk)` over-counted duplicates when a NULL primary key was present. ⚠️ Flag still defaults off pending an explicit decision to enable it in production. | Backlog (tracked in phase4c doc 6.5.11): decide whether to enable `silver_sql_dedup_enabled` in production now that integration evidence exists |
| Gold loading | Injectable `SalesGoldJob` owns batch fact build, validation, constraints, KPI, audit, versioned publication, and pointer update | Retry/reconciliation safety is adapter-dependent and should have explicit production integration evidence. 🔴 2026-09-09 finding: Gold fact batching has **no production persistence or resilience benefit** — `fact_batch_writer`/`checkpoint_manager` are accepted by `SalesGoldJob` but never wired by `_build_default_gold_job()`; every fact batch is `pd.concat()`'d into one in-memory frame and bulk-inserted once by `PostgresGoldCandidateService._insert_frame()`. Same class of issue as Silver's pre-fix state, but worse (Silver at least staged each batch to Postgres). ✅ Plan approved (scope A, phase4d 6.3.7-6.3.10): real per-batch Postgres fact writer, skip the redundant bulk insert once persisted, wire into production behind fakes-first tests plus a live-PostgreSQL integration test — not yet implemented. | Keep Gold mechanics in the job; verify adapter transaction/idempotency guarantees; implement phase4d 6.3.8-6.3.10 |
| Validation | Silver and Gold gates are enforced by `PipelineRunner` and Gold job | ✅ Completed: Full-run `PARTIAL_SUCCESS` is non-zero at CLI, and legacy `App.run()` now also maps it to `degraded` (policy decision 2026-09-09: align compatibility API with CLI severity) | ✅ `App.run()` no longer reports `PARTIAL_SUCCESS` as `ok` |
...
| Silver transformation and gate | Implemented; SQL-side path proven with live-PostgreSQL evidence, still off by default | Chunked reads, contracts, quarantine, staging, global dedup, validation, and candidate publication are present. Unsafe fallback guarded with RuntimeError; all production call sites (App, BronzeToSilverPipeline, sales_silver_clean) inject PostgresSilverPublishService correctly. **2026-09-09 correctness fix:** the deduplicated frame is now rewritten to the staging table before publish. **2026-09-09 6.5.7-6.5.10:** optional SQL-side dedup/validator eliminate the in-memory `pd.concat`/full-table pandas read entirely when both are injected; wired behind `Settings.silver_sql_dedup_enabled` (default `False`); proven against a real staged Postgres table by a new integration suite (6 tests), which caught and fixed a NULL-primary-key duplicate-count bug. Remaining backlog (6.5.11): decide when to enable the flag in production. 235 tests pass (unit + integration). |
| Gold loading and publication | Implemented; fact batching gap re-assessed, fix planned but not yet implemented | Gold job owns fact batches, validation, constraints, KPI, candidate publication, and pointer update. Adapter-level transaction/reconciliation evidence remains the production integration boundary. **2026-09-09 finding (phase4d 6.3.7):** `fact_batch_writer`/`checkpoint_manager` are unused in production wiring; every fact batch is concatenated into one in-memory frame and bulk-inserted once, so batch-sized reads give no RAM or resume benefit today. **Approved plan (6.3.8-6.3.10, not yet implemented):** real per-batch Postgres fact writer, skip the redundant bulk insert once persisted, wire into production behind fakes-first tests plus a live-PostgreSQL integration test, mirroring Silver 6.5.7-6.5.10. |
| Orchestration, CLI, and reports | Implemented | Runner, readiness gates, stage selection, report rendering, and exit-code tests are present. `App.run()` compatibility mapping now aligns with CLI: only `SUCCESS`/`SUCCESS_WITH_REJECTIONS` map to `ok`; `PARTIAL_SUCCESS` and `FAILED` both map to `degraded` (2026-09-09). |
