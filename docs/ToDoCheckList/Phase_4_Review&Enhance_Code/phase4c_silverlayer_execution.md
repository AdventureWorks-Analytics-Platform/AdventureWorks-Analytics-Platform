...
| 6.5.5 | Implement global dedup | SQL window function or equivalent over complete staging data | Duplicate keys spanning separate chunks are resolved once using deterministic `_load_date DESC, _record_hash DESC`; chunk-local dedup is not the final rule | 6.5.1/6.2.3 | Done with correctness fix (2026-09-09); pandas path remains the default, SQL-side path is opt-in (6.5.7) |
...
| 6.5.7 | Move global dedup to SQL-side on staging table (scope A+B) | `PostgresSilverDedupService.count_duplicate_keys()`/`deduplicate()` running a window-function `DELETE` against `silver_staging.<name>`; optional `dedup_service`/`staging_reader` injection points on `SilverTransformationJob` | When `dedup_service` is injected, `silver_chunks` is never accumulated during the transform loop and `_global_deduplicate()` (pandas) is bypassed entirely; default behavior (`dedup_service=None`) is unchanged for 100% backward compatibility | 6.5.5 | Done (A+B) |
| 6.5.8 | Scope C: SQL-side validator, full RAM elimination | `PostgresSilverStagingValidator.validate()`; optional `sql_validator` injection point on `SilverTransformationJob`; `PostgresSilverDedupService.set_source_snapshot_id()` | When `dedup_service` + `sql_validator` are both injected, `staging_reader` is never called and no full-table pandas frame is ever materialized for that table; schema/null-key/duplicate-key/join/threshold checks all run as SQL `COUNT`-style queries; default (`sql_validator=None`) behavior is unchanged | 6.5.7 | Done (2026-09-09) |
| 6.5.9 | Wire `PostgresSilverDedupService`/`PostgresSilverStagingValidator` into production (`App`, `BronzeToSilverPipeline`) behind a feature flag | New `Settings.silver_sql_dedup_enabled` (default `False`); `build_silver_sql_dedup_kwargs(settings)` factory in `postgres_publish_service.py`; both `App` and `BronzeToSilverPipeline` pass its result into `SalesSilverJob(**kwargs)` | Default production behavior is unchanged (`dedup_service`/`sql_validator` are `None`, pandas path runs) unless the flag is explicitly enabled; enabling it wires the real Postgres SQL services with zero code changes elsewhere | 6.5.7/6.5.8 | Done (2026-09-09), flag defaults off pending 6.5.10 |
| 6.5.10 | Add live-PostgreSQL integration test for the SQL dedup/validator path | `tests/test_postgres_silver_sql_dedup_validator_integration.py` (`@pytest.mark.integration`), exercising `PostgresSilverDedupService`/`PostgresSilverStagingValidator` against a real staging table | Cross-batch duplicate rows are removed correctly; schema/null-key/duplicate-key/join checks match the pandas equivalents on real data | 6.5.7/6.5.8 | Done (2026-09-09) |
| 6.5.11 | Fix the equivalent Gold fact `pd.concat`-before-publish pattern | Gold fact batch build reworked to avoid materializing all batches in memory before validate/publish (same class of issue as 6.5.5-6.5.8, raised for Silver but present in `sales_gold_job.py` fact staging too) | Gold fact staging no longer holds every batch in memory simultaneously before validation/publish | 6.5.7/6.5.8 pattern reference | Not started (tracked in `phase4d_goldlayer_execution.md` 6.3.7) |

Scope note (2026-09-09): 6.5.7 was decomposed into three sub-scopes. **A** (stop accumulating transformed chunks; read staging back once via `staging_reader`) and **B** (real SQL dedup via `PostgresSilverDedupService`) are implemented and unit-tested with fakes (`FakeDedupService`). **Scope C** (6.5.8, SQL-side validator) is also implemented and unit-tested with fakes (`_FakeSqlValidator`): when both `dedup_service` and `sql_validator` are injected, the job never calls `staging_reader` at all for that table, eliminating the last full-table pandas read. **6.5.9 is now wired behind `Settings.silver_sql_dedup_enabled` (default `False`):** `App` and `BronzeToSilverPipeline` both call `build_silver_sql_dedup_kwargs(settings)`, which returns `{}` unless the flag is set, so default production behavior is unchanged and no other code needed to change. **Correction:** an earlier note in this doc claimed the real SQL code could not be exercised because "no live PostgreSQL" was available in this environment — that assumption was wrong. **6.5.10 confirmed a live PostgreSQL instance is reachable here** (`pytest -m integration` passes, 22/22 at the time of the correction), and a new integration test suite (`tests/test_postgres_silver_sql_dedup_validator_integration.py`, 6 tests) now exercises `PostgresSilverDedupService`/`PostgresSilverStagingValidator` against real staged tables. **The integration test caught a real bug**: `count(*) - count(DISTINCT pk)` over-counted duplicate keys whenever a NULL primary key was present (NULL rows aren't in the `DISTINCT` set but were still counted by `count(*)`), disagreeing with the pandas `duplicated()` equivalent; fixed to `count(pk) - count(DISTINCT pk)` in both `count_duplicate_keys()` and `PostgresSilverStagingValidator.validate()`. Full suite (unit + integration): 235 passed. **The flag still defaults off** pending an explicit decision to enable it in production now that integration evidence exists. The equivalent Gold fact concat pattern (6.5.11) is tracked separately in `phase4d_goldlayer_execution.md`.
...
| 2026-09-09 | Retire unsafe `_default_writer` fallback and guard with RuntimeError | `src/features/Sales_Performance/jobs/sales_silver_job.py`, `src/app/bronze_to_silver_pipeline.py`, `scripts/transformation/silver/sales_silver_clean.py` | `pytest tests/test_silver_job.py tests/test_bronze_to_silver_pipeline.py tests/test_app_bronze_to_silver.py tests/test_architecture_contract.py -q`; `pytest tests/ -q -m "not integration"` | ✅ Guard added to `_default_writer`: raises `RuntimeError` if `publish_service is None`, preventing unsafe `to_sql(if_exists='replace')`; all 3 production call sites (App canonical wiring, BronzeToSilverPipeline default, sales_silver_clean.run()) now inject `PostgresSilverPublishService` correctly; 37 Silver tests pass (no test breakage); full non-integration suite: 198 passed | Done |
| 2026-09-09 | Fix global dedup correctness gap: deduplicated frame was discarded before publish | `sales_silver_job.py` (`_rewrite_staged_content`), `postgres_publish_service.py` (`PostgresSilverStagingWriter.replace_all`), `tests/test_silver_job.py` | `pytest tests/test_silver_job.py tests/test_postgres_publish_reconciliation.py tests/test_bronze_to_silver_pipeline.py tests/test_app_bronze_to_silver.py -q`; `pytest tests/ -q -m "not integration"` | 🔴 Found: `publish_service.publish()` renames the raw `silver_staging.<name>` table (built from per-batch `append` writes), never using the in-memory deduplicated frame from `_global_deduplicate()` — cross-batch duplicate PKs could reach published Silver in production. ✅ Fixed: after dedup, `_rewrite_staged_content()` overwrites the staged table with the deduplicated frame via a new `replace_all()` writer method before publish. New regression test `test_silver_publish_uses_deduplicated_staging_content_not_raw_batches` proves publish sees deduped content even though raw batches were staged individually. RAM usage (`pd.concat` of all chunks) is unchanged — tracked as backlog task 6.5.7 (SQL-side dedup). 200 passed. | Done |
| 2026-09-09 | Implement 6.5.7 scope A+B: optional SQL-side dedup, stop accumulating transformed chunks | `sales_silver_job.py` (`dedup_service`/`staging_reader` injection, `_default_staging_reader`, conditional dedup branch in `run()`), `postgres_publish_service.py` (`PostgresSilverDedupService.count_duplicate_keys()`/`deduplicate()`), `tests/test_silver_job.py` | `pytest tests/test_silver_job.py -q`; `pytest tests/ -q -m "not integration"` | ✅ New optional `dedup_service` parameter (default `None`, zero behavior change for existing callers/tests). When injected: `silver_chunks` is never appended during the batch loop; a SQL window-function `DELETE` runs directly on `silver_staging.<name>` via `PostgresSilverDedupService.deduplicate()`; detail-grain duplicate check runs as a SQL `count(*) - count(DISTINCT pk)` query before dedup, matching prior fail-closed semantics for `sales_order_detail`; `staging_reader` reads staging back exactly once, only for validation input. Two new focused tests with `_FakeDedupService` prove: (1) both raw batches are still staged but the dedup service — not pandas — determines `rows_deduplicated`, and staging is read back only once; (2) duplicate `sales_order_detail` grain still fails closed via the SQL count query, never reaching `deduplicate()`. `PostgresSilverDedupService`'s real SQL cannot be executed in this environment (no live PostgreSQL) — same convention as other Postgres-only services here, expected to be proven by a `@pytest.mark.integration` test against a live database. Not wired into `App`/`BronzeToSilverPipeline` yet (opt-in only); production behavior unchanged. 202 passed. | Done (scope A+B) |
| 2026-09-09 | Implement 6.5.8 scope C: SQL-side validator, eliminate the last full-table pandas read | `sales_silver_job.py` (`sql_validator` injection, `_resolve_row_count`, conditional validation block in `run()`), `postgres_publish_service.py` (`PostgresSilverStagingValidator.validate()`, `PostgresSilverDedupService.set_source_snapshot_id()`), `tests/test_silver_job.py` | `pytest tests/test_silver_job.py -q`; `pytest tests/ -q -m "not integration"` | ✅ New optional `sql_validator` parameter (default `None`, zero behavior change). When both `dedup_service` and `sql_validator` are injected: `staging_reader` is never called for that table — schema/null-key/duplicate-key/`sales_person` join/threshold checks all run as SQL `COUNT`/`information_schema` queries via `PostgresSilverStagingValidator.validate()`; `source_snapshot_id` is stamped via a SQL `UPDATE` (`PostgresSilverDedupService.set_source_snapshot_id()`) instead of a pandas column assignment; row counts in results come from the SQL validator's `target_count` via a new `_resolve_row_count()` helper. Guard: using `sql_validator` without `publish_service` raises `RuntimeError` (the legacy in-memory writer cannot run without a materialized frame). Two new focused tests with `_FakeSqlValidator` prove: (1) `staging_reader` is never called end-to-end on the full SQL path; (2) the guard raises when `publish_service` is missing. `PostgresSilverStagingValidator`'s real SQL cannot be executed in this environment (no live PostgreSQL) — same convention as other Postgres-only services here. Not wired into `App`/`BronzeToSilverPipeline` yet. 204 passed. | Done (scope C) |
| 2026-09-09 | Implement 6.5.9: wire SQL dedup/validator into production behind a feature flag | `src/core/settings.py` (`silver_sql_dedup_enabled`, default `False`), `postgres_publish_service.py` (`build_silver_sql_dedup_kwargs()`), `src/app/app.py`, `src/app/bronze_to_silver_pipeline.py`, `.env.example`, `tests/test_settings.py`, `tests/test_bronze_to_silver_pipeline.py` | `pytest tests/test_bronze_to_silver_pipeline.py tests/test_settings.py -q`; `pytest tests/ -q -m "not integration"` | ✅ New `Settings.silver_sql_dedup_enabled` flag (default `False`, documented in `.env.example` as pending 6.5.10 integration evidence). New `build_silver_sql_dedup_kwargs(settings)` factory returns `{}` unless the flag is set, else `{"dedup_service": PostgresSilverDedupService(...), "sql_validator": PostgresSilverStagingValidator(...)}`. Both `App` and `BronzeToSilverPipeline` spread its result into `SalesSilverJob(**kwargs)`, so enabling the flag requires zero further code changes. Three new focused tests: (1) `silver_sql_dedup_enabled` defaults `False` and appears in `safe_summary()`; (2) default `BronzeToSilverPipeline` construction leaves `dedup_service`/`sql_validator` as `None`; (3) enabling the flag wires real `PostgresSilverDedupService`/`PostgresSilverStagingValidator` instances. 207 passed. Flag stays off in production pending 6.5.10. | Done (2026-09-09) |
| 2026-09-09 | Implement 6.5.10: live-PostgreSQL integration test for the SQL dedup/validator path | `tests/test_postgres_silver_sql_dedup_validator_integration.py` (new, 6 tests); bugfix in `postgres_publish_service.py` (`count_duplicate_keys()`, `PostgresSilverStagingValidator.validate()`) | `pytest tests/test_postgres_silver_sql_dedup_validator_integration.py -q -m integration`; `pytest tests/ -q -m integration`; `pytest tests/ -q` (full) | 🔍 **Correction:** this doc previously assumed "no live PostgreSQL available in this environment" and marked 6.5.10 `Blocked`. That assumption was checked and was wrong — `pytest -m integration` passes (22/22) in this environment. 🔴 **Bug found by the new integration test:** `count(*) - count(DISTINCT pk)` over-counted duplicates by 1 whenever a NULL primary key existed (a table with keys `[1, NULL, 3, 3]` reported 2 duplicates instead of 1, disagreeing with the pandas `duplicated()` equivalent it replaces). ✅ **Fixed:** changed to `count(pk) - count(DISTINCT pk)` in both `PostgresSilverDedupService.count_duplicate_keys()` and `PostgresSilverStagingValidator.validate()`. New integration suite proves against a real staged table: cross-batch dedup keeps the latest row by `_load_date`/`_record_hash`; `set_source_snapshot_id()` stamps every row; null-key/duplicate-key/schema/`sales_person` join/rejected-threshold checks all match pandas semantics. Full suite (unit + integration): 235 passed. | Done (2026-09-09) |
